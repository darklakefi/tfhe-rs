...
fn main() {
    ...
    println!("Build tfhe-rocm-backend");
    println!("cargo::rerun-if-changed=hipified/include");
    println!("cargo::rerun-if-changed=hipified/src");
    println!("cargo::rerun-if-changed=hipified/tests_and_benchmarks");
    println!("cargo::rerun-if-changed=hipified/CMakeLists.txt");
    ...
    if std::env::consts::OS == "linux" {
        ...
        if distribution != "Ubuntu\n" {
            println!(
                ...
                Only Ubuntu is supported by tfhe-rocm-backend at this time. Build may fail\n"
            );
        }

        let mut cmake_config = cmake::Config::new("hipified");
        ...
        let headers = vec![
            "wrapper.h",
            "hipified/include/ciphertext.h",
            "hipified/include/integer/compression/compression.h",
            "hipified/include/integer/integer.h",
            "hipified/include/zk/zk.h",
            "hipified/include/keyswitch/keyswitch.h",
            "hipified/include/keyswitch/ks_enums.h",
            "hipified/include/linear_algebra.h",
            "hipified/include/fft/fft128.h",
            "hipified/include/pbs/programmable_bootstrap.h",
            "hipified/include/pbs/programmable_bootstrap_multibit.h",
        ];
        ...
        panic!(
            "Error: platform not supported, tfhe-rocm-backend not built (only Linux is supported)"
        );
    }
}
