...
uint64_t scratch_cuda_programmable_bootstrap_amortized_32(
  ...
  case 256:
    return scratch_programmable_bootstrap_amortized<uint32_t,
                                                    AmortizedDegree<256>>(
        static_cast<hipStream_t>(stream), gpu_index, pbs_buffer,
...
void cuda_programmable_bootstrap_amortized_lwe_ciphertext_vector_32(
  ...
  case 256:
    host_programmable_bootstrap_amortized<uint32_t, AmortizedDegree<256>>(
        static_cast<hipStream_t>(stream), gpu_index, (uint32_t *)lwe_array_out,
...
void cuda_programmable_bootstrap_amortized_lwe_ciphertext_vector_64(
  ...
  case 256:
    host_programmable_bootstrap_amortized<uint64_t, AmortizedDegree<256>>(
        static_cast<hipStream_t>(stream), gpu_index, (uint64_t *)lwe_array_out,
...
void cleanup_cuda_programmable_bootstrap_amortized(void *stream,
  ...
  // Free memory
  cuda_drop_async(*pbs_buffer, static_cast<hipStream_t>(stream), gpu_index);
}
